...
class ViewGalleryItemController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func commentTap(gesture: UIGestureRecognizer) {
        
        self.performSegue(withIdentifier: "showComments", sender: self)
    }
    
    func likeTap(gesture: UIGestureRecognizer) {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if (self.item.getComment().characters.count > 0) {
        ...
        if (self.item.getPhotoUrl().characters.count == 0) {
        ...
        if (self.item.getImgUrl().characters.count == 0) {
