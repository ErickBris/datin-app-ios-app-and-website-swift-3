...
class SignupController: UIViewController, UITextFieldDelegate, FBSDKLoginButtonDelegate {
    ...
    func signup(username: String, fullname: String, password: String, email: String) {
        ...
        let postString = "clientId=" + String(Constants.CLIENT_ID) + "&username=" + username + "&fullname=" + fullname + "&password=" + password + "&email=" + email + "&ios_fcm_regId=" + iApp.sharedInstance.getFcmRegId() + "&sex=" + String(Constants.SEX_UNKNOWN) + "&year=2000" + "&day=1" + "&month=1" + "&facebookId=" + iApp.sharedInstance.getFacebookId();
